...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - HOTPLUG - %(levelname)s - %(message)s')
    ...
    for vm_id in all_vm_metrics.keys():
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Successfully enabled hotplug (disk,network,usb,memory,cpu) for VM {vm_id}.")
                success_count += 1
                break
            except Exception as e:
                logger.error(f"Failed to enable hotplug for VM {vm_id} (Attempt {attempt+1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
                else:
                    error_count += 1
            
    logger.info(f"Hotplug Configuration Complete: {success_count} Success, {skipped_count} Skipped, {error_count} Errors.")
